...
void printRij( int [][] aTabel, int rij )
{
	for ( int kol = 0; kol < aTabel.length; kol++ )
	{
		try
		{
			int veld = veldToHexGetal( aTabel[rij][kol] );
			if ( veld < 10 )
			{
				System.out.print( " " );
			}
...
public void testTransformaties()
{
	...
	{
		Matrix matrix = MATRIX_TABEL[oktant];
		Vector translatieVector = TRANSLATIE_TABEL[oktant];
...
public void testSpiegelingInDeYas()
{
	...
	Matrix matrix = MATRIX_TABEL[2];
	Vector translatieVector = TRANSLATIE_TABEL[2];
	int [] [] getransformeerdeTabel = transformeerTabel( tabel, matrix, translatieVector );
